...
class Trainer:
    ...
    def train(self, save_ckpt=True, verbose=True):
        """
        训练
        :param save_ckpt: 是否保存权重
        :param verbose: 是否绘制曲线
        """
    ...
    def save_best_model(self, save_dir, metric = "loss", n=3, keep_last = True):
        """
        根据验证集指标保存最好的 n 个权重及最后的权重
        :param save_dir: 保存路径
        :param metric: 保存指标： loss/acc
        :param n: 保存权重个数
        :param keep_last: 是否保存最后的权重
        """
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)

        if metric == "loss":
            best_idx = np.argsort(self.valid_loss)[:n]
        elif metric == "acc":
            best_idx = np.argsort(self.valid_acc)[::-1][:n]
        else:
            raise ValueError("metric is not 'loss" or 'acc')

        best_epochs = [(index + 1) * self.eval_step for index in best_idx]
